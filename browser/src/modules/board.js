...
class Board {
  constructor(width = 19, height = 19, arrangement = [], captures = null) {
    ...
    for (let y = 0; y < this.height; y++) {
      this.arrangement[y] =
        y in arrangement ? [...arrangement[y]] : Array(this.width).fill(0);
    }
  ...
  getNeighbors(vertex, ignoreBoard = false) {
    ...
    return ignoreBoard
      ? allNeighbors
      : allNeighbors.filter((v) => this.hasVertex(v));
  }
  ...
  generateAscii() {
    ...
    this.getHandicapPlacement(9).forEach(
      (v) => (result[getIndexFromVertex(v)] = ",")
    );
    ...
    for (let { v1, v2, type } of this.lines) {
      result += `{${type === "arrow" ? "AR" : "LN"} ${this.vertex2coord(
        v1
      )} ${this.vertex2coord(v2)}}${lb}`;
    }
