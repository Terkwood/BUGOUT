...
class EngineSyncer extends EventEmitter {
  constructor(engine, multiplayer) {
    ...
    this.controller.on(
      "command-sent",
      async ({ command, getResponse, subscribe }) => {
        // Track engine state
        ...
        if (command.name === "boardsize" && command.args.length >= 1) {
          ...
          ["genmove", "lz-genmove_analyze", "genmove_analyze"].includes(
            command.name
          ) &&
  ...
  async sync(tree, id) {
    ...
    if (synced) {
      let sharedHistoryLength = [
        ...Array(Math.min(this.state.moves.length, moves.length)),
      ].findIndex((_, i) => !helper.equals(moves[i], this.state.moves[i]));
      if (sharedHistoryLength < 0)
