...
deadstones.useFetch(
  "./node_modules/@sabaki/deadstones/wasm/deadstones_bg.wasm"
);
...
class App extends Component {
  ...
  componentDidUpdate(_, prevState = {}) {
    ...
    if (representedFilename) title += " — " + this.appName;
  ...
  setMode(mode) {
    ...
    if (["scoring", "estimator"].includes(mode)) {
      ...
      deadstones
        .guess(gametree.getBoard(tree, treePosition).arrangement, {
  ...
  getEmptyGameTree() {
    ...
    let handicapStones = new Board(width, height)
    ...
    if (["play", "autoplay"].includes(this.state.mode)) {
      if (button === 0) {
        if (board.get(vertex) === 0) {
          ...
          let multiplayerColorSatisfied =
            this.state.multiplayer.yourColor &&
            this.state.multiplayer.yourColor.event &&
  ...
  getGameInfo(tree) {
    ...
    let playerRanks = ["BR", "WR"].map((x) =>
      gametree.getRootProperty(tree, x)
    );
  ...
  async syncEngines({ showErrorDialog = false } = {}) {
    ...
    try {
      while (true) {
        ...
        await Promise.all(
          this.attachedEngineSyncers.map((syncer) => {
  ...
  render(_, state) {
    ...
    return h(
      ...
      h(YourColorChosenModal, {
        yourColor: state.multiplayer && state.multiplayer.yourColor,
      }),
